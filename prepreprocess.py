...
def read_graph(filename='AbileneFlow'):
    ...
    # nodes_degree = np.array(nodes_degree / max(nodes_degree))
    # sc = MinMaxScaler(feature_range=(0, 1))
    # nodes_degree = sc.fit_transform(np.reshape(nodes_degree, (nodes_degree.shape[0], -1)))
    nodes_degree = nodes_degree.ravel()
    return Abilene_graph, nodes_degree, max(nodes_dict.keys()) + 1


if __name__ == '__main__':
    _, degree, nodes = read_graph('GeantFlow')
    print(degree)
