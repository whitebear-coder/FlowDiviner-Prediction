...
def main(filename, g_name):
    ...
    for epoch_child in range(epoch):
        ...
        print(loss)
    # ------------------------模型保存------------------
    run_name = 'GMHCN_train' + str(lr) + '_' + str(epoch) + '_bs' + str(batch_size) + '_'
    ...
    # ----------------------存到excel表格中
    data_name = 'GMHCN_test_pred' + str(lr) + '_' + str(epoch) + '_bs' + str(batch_size) + '_'
    print(test_logits.shape)
    test_logits = pd.DataFrame(test_logits)
    test_logits.to_excel(os.path.join('/home/hp/LZX/GMHCN/model_pred', data_name + '.xls'))
